...
@dynamic gpsAssistEnabled;
@dynamic gpsAssistLastUpdate;
@dynamic menuItem;
@dynamic hidden;
...
- (void)initGpsAssist
{
	...
	if (newDisk)
    {
        ...
        {
			[self sendGrowlNotification:@"A new disk is detected. Please specify what to do with it."
                                  title:newDiskNotification
                       notificationName:newDiskNotification];
	...
	if (!newDisk)
    {
		[self sendGrowlNotification:[path lastPathComponent]
                              title:mountNotification
                   notificationName:mountNotification];
	}
...
- (void)gpsAssistDataAvailable:(NSData*)gpsAssistData date:(NSDate*)downloadDate
{
    ...
    else
    {
		...
		// Growl it
		[self sendGrowlNotification:[NSString stringWithFormat:@"Disk %@: valid until %@", [path lastPathComponent], [self validTimeAsString]]
                              title:updatedNotification
                   notificationName:updatedNotification];
	}
}

- (void)gpsAssistDataFailed
{
	[self sendGrowlNotification:[NSString stringWithFormat:@"GPSAssist-data update failed for disk %@", [path lastPathComponent]]
                          title:updateFailedNotification
               notificationName:updateFailedNotification];
